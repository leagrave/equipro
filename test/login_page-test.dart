...
void main() {
  testWidgets('Test de la page de connexion avec GoRouter', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp.router(
        routerConfig: go, // Utilisation du GoRouter configuré
      ),
    );
    ...
    expect(find.byType(GestureDetector), findsNWidgets(2)); // Liens "Mot de passe oublié" et "Créer un compte"
    ...
    // Vérifier la navigation après le clic sur "Se connecter"
    expect(find.text('Page principale'), findsOneWidget); // Vérifie la page cible, ici tu peux ajuster selon ta logique de navigation
  });
}
